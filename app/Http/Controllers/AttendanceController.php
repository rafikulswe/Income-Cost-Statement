...
class AttendanceController extends Controller
{
    ...
    {
        $user_id = Auth::user()->id;
        $lastSemesterId = Semisters::where('employee_id', $user_id)->where('valid', 1)->max('id');
        $startDate = Semisters::where('id', $lastSemesterId)->where('valid', 1)->first()->start_date;
        $data['startDate'] = DateTime::createFromFormat('Y-m-d', $startDate)->format('m/d/Y');
        return view('web.attendance.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        $day_name = new DateTime($attend_date);
        $day = $day_name->format('l');
        $semesterSchedule = EmployeeSchedules::join('schedule_days', 'schedule_days.day_index', '=', 'employee_schedules.day_index')
            ->select('employee_schedules.*', 'schedule_days.day_name')
            ->where('employee_schedules.employee_id', $user_id)
            ->where('schedule_days.day_name', $day)
            ->where('employee_schedules.valid', 1)
            ->get();
        foreach ($semesterSchedule as $key => $semesterSchedule) {
            $inserted_start_time = Carbon::parse($start_time);
            $schedule_start_time = Carbon::parse($semesterSchedule->start_time);
            $diff_in_minutes = $inserted_start_time->diffInMinutes($schedule_start_time);
            $late_duration = gmdate('H:i:s', $diff_in_minutes);
        }
    ...
    {
        $data['employees'] = User::where('valid', 1)->where('is_employee', 1)->get();
