...
class SupervisorController extends Controller
{
    ...
    public function create(){
        $supervisor_id = User::max('id')+1;
        $id_len = strlen($supervisor_id);
        if($id_len == 1){
            $data['generate_emp_id'] = '00'.$supervisor_id;
        } elseif($id_len == 2){
            $data['generate_emp_id'] = '0'.$supervisor_id;
        } else {
            $data['generate_emp_id'] = $supervisor_id;
        }
